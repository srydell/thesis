...
for i in range(N):
    for j in range(M):
        ...
        leftNode = (i-1, j)
        aboveNode = (i, j-1)
        leftLabel = graph.get(leftNode).get("label")
        aboveLabel = graph.get(aboveNode).get("label")

        # Neither a label above nor to the left.
        if (leftLabel == 0) and (aboveLabel == 0)
            # Make a new, as-yet-unused cluster label.
            largest_label = largest_label + 1
            graph[(i,j)].get("label") = largest_label;

        elif (leftLabel != 0) and (aboveLabel == 0)
            # One neighbor, to the left.
            graph[(i,j)].get("label") = find(leftLabel);

        elif (leftLabel == 0) and (aboveLabel != 0)
            # One neighbor, above.
            graph[(i,j)].get("label") = find(aboveLabel);
