...
\maketitle
% My name is Simon Rydell and this is my master thesis that was made here for the last 8 months. It is called \title and I will start off by giving a small overview of what I will present.

\begin{frame}{Table of contents}
  \setbeamertemplate{section in toc}[sections numbered]
  \tableofcontents[hideallsubsections]
\end{frame}
% This talk is divided into four parts. I will first expand a bit on what a fractal is, what fractal dimension is and so on. Then talk about the algorithms used for generating and working with the patterns that will be examined, and then finally talk about the two models used in this thesis.
...
% TODO: Create a divide on the second frame where there are four boxes and three sierpinski triangles
...
% Given enough sampling, a physical system will emerge. This can then be divided as per the discussed box counting method. The x-axis here shows the relative size of the box division, so a relative size of 1 would be the whole system. The result is here compared to the theoretical value given by Loewner expansion. So we can see a somewhat close approximation. {Switch slide}
...
% If we instead use the scaling method, the result can be seen in this loglog plot. Here the theoretical value is within the scope of the error. The red line here is just for comparison. So, if we compare all the results together... {Switch slide}
